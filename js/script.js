...
var no_rooms = 0;
var distance_coverd = 0;
var packing_services = 0;
...
$(function() {
    $('input[type="range"]').rangeslider({
        polyfill: false,
        onInit: function() {
            $(".header .pull-right").text($('input[type="range"]').val() + " miles");
        },
        onSlide: function(position, value) {
            //console.log('onSlide');
            //console.log('position: ' + position, 'value: ' + value);
            $(".header .pull-right").text(value + " miles");
        },
...
function handleslidevalueChange() {
    let slideVal = parseInt($("#rng-dse").val());
    let roomValue = parseInt($("#noofroom").val());
    let sbtotal = 8;

    $("#lnrooms").text(roomValue);
    $("#lndst").text(slideVal);
...
const postQuote = async() => {
    ...
    let pstbtn = document.getElementById("pstbtn");

    if (firstName.value === "" || lastName.value === "" || email.value === "" || rooms.value === "" || phoneNumber.value === "" || currentAddress.value === "" || newAddress.value === "" || date1.value === "") {

        resultID.innerHTML = "One of the required field is empty";
        resultID.classList.add("reuiredcst")

    } else {
        resultID.innerHTML = "Get in touch with us if you have any queries and we will get back to you as soon as possible.";
        resultID.classList.remove("reuiredcst");
        pstbtn.innerHTML = "Please Wait";
        pstbtn.onclick = null;
        try {
            let data1 = {
            ...
            console.log(data1)
            const response = await axios.post(`${BASE_URL}/quote`, data1);

            const { data, status } = response;
            console.log(status, "asdas")
            if (status == 200) {
                resultID.classList.add("reuiredcst")
                resultID.innerHTML = "Request Submitted Successfully";
                pstbtn.innerHTML = "Success";


            } else {
                pstbtn.innerHTML = "Request Quote";
                resultID.classList.add("reuiredcst")
                resultID.innerHTML = "Something wen't wrong please try again";
                pstbtn.addEventListener("click", () => postQuote());
            }
